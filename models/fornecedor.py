...
class Fornecedor:
    def __init__(self, id, nome, contato, endereco, cnpj):
        self.id = id
        self.nome = nome
...
class FornecedorRepository:
    ...
    def get_all(self):
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT id, nome, contato, endereco, cnpj FROM fornecedores")
            return [self._row_to_fornecedor(row) for row in cursor.fetchall()]

    def get_by_id(self, fornecedor_id):
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT id, nome, contato, endereco, cnpj FROM fornecedores WHERE id = ?", (fornecedor_id,))
            row = cursor.fetchone()
    ...
    def add_fornecedor(self, fornecedor):
        with self._get_connection() as conn:
            ...
            cursor.execute(
                ...
                (fornecedor.nome, fornecedor.contato, fornecedor.endereco, fornecedor.cnpj)
            )
            conn.commit()
    ...
    def update_fornecedor(self, fornecedor):
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE fornecedores SET nome = ?, contato = ?, endereco = ?, cnpj = ? WHERE id = ?",
                (fornecedor.nome, fornecedor.contato, fornecedor.endereco, fornecedor.cnpj, fornecedor.id)
            )
            conn.commit()
